...
private BSearchDto sdto; // page, perPageNum
	...
	private Integer displayPageNum = 10; // 페이지네이션 보여줄 갯 수 10개로 고정
	
	// 리스트 페이지에서 단일 게시글 클릭하면 페이징, 검색 정보를 가지고 URI문자열을 만들어서 조회페이지로 이동한다. 
		// 그러면 조회페이지에서 페이징, 검색정보를 유지하고 있기 때문에 다시 리스트 페이지로 이동할 때 원래 page와 검색 조건의 리스트로 이동할 수 있다.
		...
		// 리스트페이지에서 단일 게시글 클릭하면 해당 page정보를 가지고 page, perPageNum 파라미터를 포함한 URI 문자열을 만들어서 조회페이지로 
		// 이동한다. 그러면 조회페이지에서 page, perPageNum, bno 값을 유지하고 있기 때문에 다시 리스트 페이지로 이동할때 원래 page로 이동할 수 있다.
